...
/* ── BSS 静态池（禁止运行时动态分配） ── */
static ws2812_priv_t s_ws2812_pool[WS2812_COUNT];
...
static int ws2812_probe(device_t* dev)
{
    ...
    pool_idx = osal_pool_claim(s_ws2812_used, WS2812_COUNT);
    if (pool_idx < 0) {
        ret = VFS_ERR_NOMEM;
        goto err_pool;
    }
...
err_pool:
    if (pool_idx >= 0) { osal_pool_release(s_ws2812_used, WS2812_COUNT, pool_idx); }
    return ret;
}
...
static int ws2812_remove(device_t* dev)
{
    ...
    if (priv) {
        ...
        osal_pool_release(s_ws2812_used, WS2812_COUNT, priv->pool_idx);
...
static int ws2812_ioctl(device_t* dev, int cmd, void* arg, size_t arg_len, uint32_t timeout_ms)
{
    ...
    case WS2812_CMD_SET_COLOR:
        if (arg_len != sizeof(ws2812_color_t) || !arg) { ret = VFS_ERR_INVAL; break; }
        {
    ...
    case WS2812_CMD_SET_BRIGHTNESS:
        if (arg_len != sizeof(uint8_t) || !arg) { ret = VFS_ERR_INVAL; break; }
