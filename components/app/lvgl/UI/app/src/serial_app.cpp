...
void SerialApp::build_ui()
{
    ...
    lv_obj_set_style_bg_color(m_screen, lv_color_hex(CLR_BG), 0);
    ...
    lv_obj_set_style_bg_color(hdr, lv_color_hex(0x161822), 0);
    ...
    lv_obj_set_style_text_color(title, lv_color_hex(CLR_WHITE), 0);
    ...
    lv_obj_set_style_text_color(m_conn_dot, lv_color_hex(CLR_GRAY2), 0);
    ...
    lv_obj_set_style_bg_color(m_output, lv_color_hex(0x08090C), 0);
    lv_obj_set_style_bg_opa(m_output, LV_OPA_COVER, 0);
    lv_obj_set_style_border_width(m_output, 1, 0);
    lv_obj_set_style_border_color(m_output, lv_color_hex(CLR_BORDER), 0);
    ...
    lv_obj_set_style_bg_color(sta, lv_color_hex(0x161822), 0);
    lv_obj_set_style_bg_opa(sta, LV_OPA_COVER, 0);
    lv_obj_set_style_border_width(sta, 1, 0);
    lv_obj_set_style_border_color(sta, lv_color_hex(CLR_BORDER), 0);
    ...
    lv_obj_set_style_text_color(m_baud_val, lv_color_hex(CLR_GRAY), 0);
    ...
    lv_obj_set_style_text_color(m_term_val, lv_color_hex(CLR_GRAY), 0);
    ...
    /* 查看历史 — 文字+透明容器 */
    {
        ...
        lv_obj_set_style_border_color(cont, lv_color_hex(CLR_BORDER), 0);
        ...
        lv_obj_set_style_text_color(m_view_lbl, lv_color_hex(CLR_GRAY), 0);
    ...
    /* 清屏 — 文字+透明容器 */
    {
        ...
        lv_obj_set_style_border_color(cont2, lv_color_hex(CLR_BORDER), 0);
        ...
        lv_obj_set_style_text_color(m_clear_lbl, lv_color_hex(CLR_GRAY), 0);
...
void SerialApp::focus_update()
{
    ...
    reset_text(m_baud_val, CLR_GRAY);
    reset_text(m_term_val, CLR_GRAY);
    reset_text(m_view_lbl, CLR_GRAY);
    reset_text(m_clear_lbl, CLR_GRAY);
    reset_border(m_view_lbl, 1, CLR_BORDER);
    reset_border(m_clear_lbl, 1, CLR_BORDER);

    lv_color_t accent = m_adjust ? lv_color_hex(0xFFFFFF) : lv_color_hex(CLR_ORANGE);
...
void SerialApp::handle_key(uint32_t key)
{
    ...
    if (m_adjust) {
        ...
        switch (m_focus) {
        case FOCUS_BAUD:
            if (key == LV_KEY_NEXT) { m_baud_idx = (m_baud_idx + 1) % BAUD_COUNT; on_baud_change(m_baud_idx); update_display(); }
            else if (key == LV_KEY_PREV) { m_baud_idx = (m_baud_idx + BAUD_COUNT - 1) % BAUD_COUNT; on_baud_change(m_baud_idx); update_display(); }
            break;
        case FOCUS_TERM_MODE:
            if (key == LV_KEY_NEXT) { m_term_mode = (m_term_mode + 1) % TERM_MODE_COUNT; on_term_mode_change(m_term_mode); update_display(); }
            else if (key == LV_KEY_PREV) { m_term_mode = (m_term_mode + TERM_MODE_COUNT - 1) % TERM_MODE_COUNT; on_term_mode_change(m_term_mode); update_display(); }
            break;
        case FOCUS_VIEW:
        ...
        if (key == LV_KEY_ENTER) { m_adjust = true; focus_update(); }
...
void SerialApp::hide()
{
    ...
    if (m_refr_timer) { lv_timer_delete(m_refr_timer); m_refr_timer = nullptr; }
