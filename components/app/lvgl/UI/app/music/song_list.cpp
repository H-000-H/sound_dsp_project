...
void SongListPage::build_ui()
{
    ...
    lv_obj_set_style_text_color(title, lv_color_hex(0xF5F5F7), 0);
...
void SongListPage::update_list()
{
    ...
    for (int i = start; i < end; i++) {
        ...
        lv_obj_set_style_bg_color(row, lv_color_hex(0x1C1C22), 0);
        lv_obj_set_style_bg_opa(row, LV_OPA_COVER, 0);
        lv_obj_set_style_border_width(row, 1, 0);
        lv_obj_set_style_border_color(row, lv_color_hex(0x2C2F3E), 0);
        ...
        lv_obj_set_style_text_color(idx_lbl, lv_color_hex(0x5C5F73), 0);
        ...
        lv_obj_set_style_text_color(name_lbl, lv_color_hex(0xF5F5F7), 0);
        lv_obj_set_style_text_font(name_lbl, &lv_font_custom_16, 0);
        lv_obj_align(name_lbl, LV_ALIGN_LEFT_MID, 36, 0);

        if (i - start == m_focus) {
            lv_obj_set_style_bg_color(row, lv_color_hex(0x2C2F3E), 0);
            lv_obj_set_style_border_color(row, lv_color_hex(0xFF9F0A), 0);
    ...
    if (m_dots) {
        ...
        for (int p = 0; p < pages; p++) {
            ...
            lv_obj_set_style_bg_color(dot,
                p == m_page ? lv_color_hex(0xFF9F0A) : lv_color_hex(0x3A3D4E), 0);
...
void SongListPage::song_list_nav_timer_cb(lv_timer_t* t)
{
    ...
    if (gpio < 0) { self->m_nav_held = false; return; }
